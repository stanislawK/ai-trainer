...
| [tests.md](tests.md) | `tests/**/*.py` | TDD (red → green → refactor), unit/integration/e2e split, no network (models.ALLOW_MODEL_REQUESTS = False), frozen time, fakes over mocks, tenancy tests, MCP tools via FastMCP Client(), pytest-asyncio auto mode, e2e asserts behavior not pixels. |
| [web.md](web.md) | `src/ai_trainer/web/**` | htmx 4 (not 2), type→feature template layout, partials for HX-Request, daisyUI 5 + Tailwind with the Liquid Glass theme and glass utilities only (ADR-0019), built from the Claude Design mock via `/implement-design`, mobile first, dark default, skeletons and the thinking indicator, SSE streams, user strings in templates, CSRF on POST, AI changes as confirm/edit/discard cards, ambiguity as choice card with option IDs. |
