...
# Add an endpoint (ADR-0003, ADR-0005, ADR-0012, ADR-0019)
...
5. HTML: a full page from `templates/pages/<feature>/` for normal requests, a partial from `templates/partials/<feature>/` for `HX-Request`, extending `layouts/app.html` (signed in) or `layouts/bare.html` (ADR-0019). A designed screen is built with `/implement-design`. Streaming uses `fastapi.sse.EventSourceResponse`.
6. Tests (via `/run-tdd`):
