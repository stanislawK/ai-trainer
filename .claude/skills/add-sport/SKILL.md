...
1. Create `src/ai_trainer/domain/sports/<sport>/` with `payload.py` (payload model + version), `normalize.py` (units and scales), `load.py` (load calculator emitting `TrainingLoad` with the common system vocabulary) and `plugin.py`.
   - **Fan out, then reconcile.** Before writing the files, write a short shared spec: the sport's payload fields, units/scales, and the load formula's inputs/outputs. Draft the four files with one agent per file, in parallel, each given only the shared spec. Then run one synthesis pass that reads all four files plus the spec and confirms field names, units and types agree across them before moving on — fix mismatches here, not in step 5's tests.
