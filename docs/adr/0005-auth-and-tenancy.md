...
- **Every state-changing request is CSRF-checked** (#15) with a stateless, signed token bound to the session: the HMAC-SHA256 of the session id under a dedicated `csrf_secret_key` setting, kept separate from `session_secret_key` so the two signers don't share a key. `CsrfMiddleware` computes `request.state.csrf_token` from the `session_id` cookie on every request (`None` without one) so templates can expose it without knowing how it's derived, and rejects any non-`GET`/`HEAD`/`OPTIONS` request with `403` unless it carries a matching value in the `X-CSRF-Token` header. Nothing is stored server-side; the server just recomputes the expected token and compares. `layouts/base.html`'s `<html>` tag sets `hx-headers:inherited='{"X-CSRF-Token": "..."}'` (htmx 4's documented pattern, `/bigskysoftware/htmx/v4.0.0`), so every htmx-issued request from a rendered page carries the header automatically with no per-form wiring. A custom header defeats CSRF here because a cross-site form can't set one, and a cross-origin `fetch` that tries gets blocked by CORS since the app grants none.
...
3. Every request other than GET, HEAD or OPTIONS is CSRF-checked.
